...
public extension ASAppleMusic {
     ...
     */
    func getSong(withID id: String, lang: String? = nil, completion: @escaping (_ song: AMLibrarySong?, _ error: AMError?) -> Void) {
     ...
     */
    func getMultipleSongs(withIDs ids: [String]? = nil, lang: String? = nil, completion: @escaping (_ songs: [AMLibrarySong]?, _ error: AMError?) -> Void) {
        callWithToken { token in
            guard let token = token else {
                let error = AMError()
                error.status = "401"
